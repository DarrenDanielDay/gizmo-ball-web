...
export class PipeMapItem extends MapItem implements IRotatable {
  ...
  public crashHandle(crashable: ICollisible) {
    ...
    if (crashable instanceof Ball) {
      ...
      let ballPos: Vector2D = crashable.massPoint.p.add(massPoint.v.mult(-Physical.tick));
      // 获取碰撞边
      ...
      if (crashLine !== null) {
        ...
        if (crashLine.p1.equals(edges[0].p1) || crashLine.p1.equals(edges[2].p1)) {
          // 侧边
          // console.log("side!");
          ...
          if (v.dot(axis) < 0) {
            // 准备进入管道
            // console.log("entry!");
            // eslint-disable-next-line es-beautifier/multiline-array-elements
            const lineCenter:Vector2D = Vector2D.center([crashLine.p1, crashLine.p2]);
            const ballCenter:Vector2D = crashable.center;
            if (Math.abs(Vector2D.difference(ballCenter, lineCenter).projection(crashLine.directionVector)) < 10) {
              // console.log("move!");
              // 若从其它管道进入，则不改变速度大小，只改变方向
              if (crashable.pipeID !== -1) {
                massPoint.setVelocity(v.component(axis).mult(v.radius / Math.abs(v.projection(axis))));
              } else {
                massPoint.setVelocity(v.component(axis));
              }
              crashable.pipeID = this.id;
              const change: Vector2D = Vector2D.difference(lineCenter, ballCenter).component(crashLine.directionVector);
